"""Claim-check v3: HDGPSO with surrogate + full-budget eval cap.

Same task config as v2 — same 4 datasets, 4 models, 7 tuners, 3 seeds = 189 cells.
The only change vs v2 is HDGPSO itself (surrogate-assisted + eval_budget cap).

Writes to results_claim_check_v3/.
"""
...
OUT_DIR = "results_claim_check_v3"
...
def check_claims(summary: pd.DataFrame) -> tuple[dict, str]:
    ...
    out("=" * 70)
    out("PAPER CLAIM CHECK v3 (surrogate-assisted HDGPSO + full eval budget)")
    out("=" * 70)
...
def main():
    ...
    cd_diagram(summary, save_path=os.path.join(OUT_DIR, "fig_cd_diagram.png"),
               title=f"CD diagram v3 (n={len(build_rank_matrix(summary))} cells)")
